...
def generate_controller_delete_file(base_path, singular_name, plural_name, singular_name_kebab, plural_name_kebab, singular_name_snake, plural_name_snake, namespace, app_name):
    ...
    controller_file_path = os.path.join(controller_path, f'{singular_name_snake}_delete_controller.py')
...
class {singular_name}DeleteController(View):

    def delete(self, request, {singular_name_snake}_id):
        
        repository = {singular_name}Repository()
        deleted_record = repository.delete({singular_name_snake}_id)

        if deleted_record:
            return JsonResponse({{
                'success': True,
                'message': '{singular_name} deleted successfully',
                'data': None 
            }}, status=200)
        else:
            return JsonResponse({{
    ...
    try:
        with open(controller_file_path, 'w') as controller_file:
            ...
            print(f"Archivo de controlador '{singular_name_snake}_delete_controller.py' creado en: {controller_file_path}")
    except Exception as e:
        print(f"Error al crear el archivo de controlador '{singular_name_snake}_delete_controller.py': {e}")
